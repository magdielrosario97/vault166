...
class Game:
    ...
    def process_command(self, command: str) -> None:
        ...
        if command.startswith("go "):
            direction = command[3:]
            if direction in self.room_connections[current_room]:
                self.player.current_room = self.room_connections[current_room][direction]
                current_room = self.player.current_room
                # Check if the player encounters Maradonyx
                if current_room == "Chemical Room":
                    print(self.rooms[current_room])
                    if len(self.player.inventory) == 7 and current_room == 'Chemical Room':
                ...
                print(f'HINT: {self.room_connections[current_room]}')
        elif command.startswith("get "):
            item = command[4:]
            if self.items[current_room] == item:
                self.player.inventory.append(item)
                print("You picked up the\033[94m", item + "\033[0m.")
                self.items[current_room] = None
            else:
